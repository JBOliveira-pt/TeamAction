...
function KpiCard({
    ...
    return (
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 shadow-sm border border-gray-200 dark:border-gray-800 flex flex-col gap-2">
            ...
            <span className={`text-3xl font-bold font-mono ${color ?? 'text-gray-900 dark:text-white'}`}>
                {value}
            </span>
...
export default async function PaiDashboard() {
    ...
    const nome = dados?.atleta?.nome ?? dados?.minorUser?.name ?? '—';
    const posicao = dados?.atleta?.posicao ?? null;
    const numero = dados?.atleta?.numero_camisola ?? null;
    const equipa = dados?.atleta?.equipa_nome ?? null;
    const mao = dados?.atleta?.mao_dominante ?? null;
    const estado = dados?.atleta?.estado ?? null;
    ...
    return (
        <main className="p-6 space-y-6 bg-gray-50 dark:bg-gray-950 min-h-screen">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {(posicao || numero != null || equipa) && (
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                            {[posicao, numero != null ? `Nº ${numero}` : null, equipa]
                                .filter(Boolean)
                                .join(' · ')}
                        </p>
                    )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <KpiCard
                    title="Golos Época"
                    value={totalJogos > 0 ? String(totalGolos) : '—'}
                    sub={mediaGolos ? `${mediaGolos} / jogo` : 'Sem dados'}
                />
                <KpiCard
                    title="Assistências"
                    value={totalJogos > 0 ? String(totalAssistencias) : '—'}
                    sub={totalJogos > 0 ? `${totalJogos} jogos` : 'Sem dados'}
                />
                <KpiCard
                    title="Min. Jogados"
                    value={totalJogos > 0 ? String(totalMinutos) : '—'}
                    sub={totalJogos > 0 ? `${totalJogos} jogos` : 'Sem dados'}
                />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 shadow-sm border border-gray-200 dark:border-gray-800 flex flex-col items-center justify-center gap-2 text-center">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                        ...
                        para ver os dados físicos do teu filho.
                    </p>
                </div>
                <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 shadow-sm border border-gray-200 dark:border-gray-800 flex flex-col items-center justify-center gap-2 text-center">
                    <span className="text-2xl">📅</span>
