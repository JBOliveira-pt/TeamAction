...
type Usuario = { id: string; name: string; email: string; image_url: string | null };

export default function ConvidarAtletaModal({ equipas }: { equipas: Equipa[] }) {
    const [open, setOpen]       = useState(false);
    const [state, action, isPending] = useActionState<State, FormData>(convidarAtleta, null);
    ...
    useEffect(() => {
        if (!searchQuery || searchQuery.length < 2) { setResultados([]); return; }
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-md bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl p-6 space-y-5">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <h2 className="text-lg font-bold text-gray-900 dark:text-white">Convidar Atleta</h2>
                        ...
                        <form ref={formRef} action={action} className="space-y-4">
                            <input type="hidden" name="atleta_user_id" value={selecionado?.id ?? ""} />
                            ...
                            <div className="space-y-1">
                                ...
                                {selecionado ? (
                                    <div className="flex items-center justify-between bg-blue-500/10 border border-blue-500/30 rounded-lg px-4 py-3">
                                        <div className="flex items-center gap-3">
                                            {selecionado.image_url ? (
                                                <img src={selecionado.image_url} alt={selecionado.name} className="w-8 h-8 rounded-full object-cover" />
                                            ) : (
                                            ...
                                            <div>
                                                <p className="text-sm font-semibold text-blue-400">{selecionado.name}</p>
                                                <p className="text-xs text-gray-400">{selecionado.email}</p>
                                            </div>
                                        </div>
                                        <button type="button" onClick={() => setSelecionado(null)} className="text-gray-400 hover:text-gray-200 transition-colors">
                                            <X size={14} />
                                        </button>
                                    </div>
                                ) : (
                                    <div className="relative">
                                            ...
                                            <div className="absolute top-full mt-1 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-20 overflow-hidden">
                                                {resultados.map((u) => (
                                                    <button
                                                        key={u.id}
                                                        type="button"
                                                        onClick={() => { setSelecionado(u); setSearchQuery(""); setResultados([]); }}
                                                        className="w-full text-left px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex items-center gap-3"
                                                    >
                                                        {u.image_url ? (
                                                            <img src={u.image_url} alt={u.name} className="w-7 h-7 rounded-full object-cover flex-shrink-0" />
                                                        ) : (
                                        ...
                                        {isSearching && <p className="text-xs text-gray-400 mt-1">A pesquisar...</p>}
                                        {!isSearching && searchQuery.length >= 2 && resultados.length === 0 && (
                                            <p className="text-xs text-gray-400 mt-1">Nenhum utilizador encontrado.</p>
                                        )}
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="px-4 py-3 bg-amber-500/10 border border-amber-500/20 rounded-lg text-xs text-amber-400">
                                O atleta receberá uma notificação e terá de aceitar o convite para ser federado no clube.
                            </div>
