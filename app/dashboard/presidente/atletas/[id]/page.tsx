...
export default async function AtletaDetailPage({
    ...
    const [{ atleta, mensalidades, estatisticas, assiduidade }, equipas] =
        await Promise.all([fetchAtletaById(id), fetchEquipas()]);
    ...
    const percAssiduidade =
        totalTreinos > 0 ? Math.round((presencas / totalTreinos) * 100) : 0;
    ...
    const estatisticasCards = [
        ...
        {
            label: "Assistências",
            valor: Number(estatisticas?.total_assistencias ?? 0),
        },
        {
            label: "Cart. Amarelos",
            valor: Number(estatisticas?.total_cartoes_amarelos ?? 0),
        },
        {
            label: "Cart. Vermelhos",
            valor: Number(estatisticas?.total_cartoes_vermelhos ?? 0),
        },
        ...
        <div className="p-6 space-y-6">
            ...
            <div>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                                {atleta.nome}
                                {!atleta.user_id && (
                                    <span className="ml-2 px-2 py-0.5 rounded text-[10px] font-semibold bg-gray-500/10 text-gray-400 border border-gray-500/20 align-middle">
                                        🤖 Fictício
                                    </span>
                                )}
                            </h1>
                    ...
                    <div className="flex items-center gap-2">
                        <span
                            className={`px-3 py-1.5 rounded-full text-xs font-semibold ${estadoStyle[atleta.estado] ?? "bg-slate-500/10 text-slate-400"}`}
                        >
                            {atleta.estado}
                        </span>
                        {atleta.user_id && mensalidadeAtual && (
                            ...
                            >
                                {mensalidadeLabel[mensalidadeAtual.estado] ??
                                    mensalidadeAtual.estado}
                            </span>
                        )}
                        <EditarAtletaModal
                            atleta={{
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                {estatisticasCards.map((stat) => (
                    ...
                    >
                        <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider leading-tight">
                            {stat.label}
                        </p>
                    ...
                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 space-y-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-medium text-gray-900 dark:text-white mt-0.5">
                                    {atleta.federado
                                        ? `Sim — ${atleta.numero_federado ?? "s/n"}`
                                        : "Não"}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <span
                                    className={`inline-flex px-2 py-1 rounded-full text-xs font-medium mt-0.5 ${estadoStyle[atleta.estado] ?? "bg-slate-500/10 text-slate-400"}`}
                                >
                                    {atleta.estado}
                                </span>
                            </div>
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-lg font-bold text-cyan-400 w-14 text-right">
                                {percAssiduidade}%
                            </span>
                        </div>
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
                            ...
                            {percAssiduidade >= 90
                                ? "✅ Excelente assiduidade"
                                : percAssiduidade >= 75
                                  ? "⚠️ Assiduidade razoável"
                                  : "🔴 Assiduidade baixa"}
                        </p>
                    </div>
