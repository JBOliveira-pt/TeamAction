...
export default async function EquipaDetailPage({
    ...
    const derrotas = jogosRealizados.filter(
        (j) =>
            j.resultado_nos != null &&
            j.resultado_adv != null &&
            j.resultado_nos < j.resultado_adv,
    ).length;
    const empates = jogosRealizados.filter(
        (j) =>
            j.resultado_nos != null &&
            j.resultado_adv != null &&
            j.resultado_nos === j.resultado_adv,
    ).length;
    const golosMarcados = jogosRealizados.reduce(
        (acc, j) => acc + (j.resultado_nos ?? 0),
        0,
    );
    const golosSofridos = jogosRealizados.reduce(
        (acc, j) => acc + (j.resultado_adv ?? 0),
        0,
    );
    const pontosPerc =
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            {equipa.escalao} · {equipa.desporto}
                        </p>
                    </div>
                    <span
                                ...
                                <tbody>
                                    {atletas.map((atleta) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-3">
                                                <Link
                                                    href={`/dashboard/presidente/atletas/${atleta.id}`}
