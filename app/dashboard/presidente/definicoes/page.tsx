...
export default async function DefinicoesPage() {
    ...
    if (!org) {
        return (
            <div className="p-6">
                <p className="text-gray-400 dark:text-gray-500 text-sm">Organização não encontrada.</p>
            </div>
        );
    }
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 space-y-5">
                <div className="flex items-center justify-between">
                    ...
                    {org.plano && (
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${planoStyle[org.plano] ?? "bg-gray-500/10 text-gray-400"}`}>
                            Plano {org.plano}
                        </span>
                    )}
                </div>
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 space-y-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-sm font-semibold text-gray-900 dark:text-white">📅 Época Ativa</h2>
                    <a
                ...
                {epoca ? (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
                            ...
                            <p className="text-sm font-semibold text-gray-900 dark:text-white mt-0.5">{formatData(epoca.data_inicio)}</p>
                        </div>
                        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
                            <p className="text-xs text-gray-400 dark:text-gray-500">Fim</p>
                            <p className="text-sm font-semibold text-gray-900 dark:text-white mt-0.5">{formatData(epoca.data_fim)}</p>
                        </div>
                    </div>
                ) : (
                    <div className="flex items-center justify-between bg-amber-500/5 border border-amber-500/20 rounded-lg p-4">
                        <p className="text-sm text-amber-400">Nenhuma época ativa definida.</p>
                        <a
                            href="/dashboard/presidente/epoca"
