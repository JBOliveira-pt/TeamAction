...
type Membro = { id: string; nome: string; funcao: string; equipaid: string | null; userid: string | null }
...
export function EditarMembroModal({ membro, equipas, users }: {
  membro: Membro
  equipas: Equipa[]
  ...
  const [selectedUser, setSelectedUser] = useState<UserPlataforma | null>(currentUser)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl p-6 space-y-5 max-h-[90vh] overflow-y-auto">
            ...
            <form ref={formRef} action={action} className="space-y-4">
              ...
              {/* Treinador → pesquisar utilizador */}
              {isTreinador && (
                <div className="space-y-2">
                  <label className="text-xs font-medium text-gray-500 dark:text-gray-400">
                    Treinador na plataforma <span className="text-red-400">*</span>
                  </label>
                  ...
                  <input type="hidden" name="nome" value={selectedUser?.name ?? membro.nome} />
                  ...
                  {selectedUser && !loadingEscaloes && (
                    <div className="px-3 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg">
                      <p className="text-xs text-gray-500 dark:text-gray-400 mb-1.5">Escalões cobertos:</p>
                      {escaloesTreinador.length > 0 ? (
                        <div className="flex flex-wrap gap-1.5">
                          {escaloesTreinador.map(e => (
                            <span key={e} className="px-2 py-0.5 bg-violet-500/10 border border-violet-500/20 text-violet-400 text-xs rounded-full">{e}</span>
                          ))}
                        </div>
                      ) : (
                        <p className="text-xs text-red-400">⚠️ Sem cursos registados na plataforma.</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-1">
                ...
                {isTreinador && escaloesTreinador.length > 0 && equipasFiltradas.length < equipas.length && (
                  <p className="text-xs text-gray-400">A mostrar apenas equipas dos escalões cobertos pelo curso.</p>
                )}
              </div>

              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isPending || (isTreinador && !selectedUser)}
