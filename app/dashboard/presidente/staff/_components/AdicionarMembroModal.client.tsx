...
type UserPlataforma = { id: string; name: string; email: string; imageurl: string | null }

const FUNCOES = [
  'Treinador Principal', 'Treinador Adjunto',
  'Fisioterapeuta', 'Preparador Físico', 'Team Manager',
  'Médico', 'Nutricionista', 'Delegado', 'Outro',
]
const FUNCOES_TREINADOR = ['Treinador Principal', 'Treinador Adjunto']

export function AdicionarMembroModal({ equipas, users }: { equipas: Equipa[]; users: UserPlataforma[] }) {
  ...
  const [selectedUser, setSelectedUser] = useState<UserPlataforma | null>(null)
  ...
  const equipasFiltradas = isTreinador && escaloesTreinador.length > 0
  ...
  async function handleSelectUser(u: UserPlataforma) {
    ...
    // Buscar escalões cobertos pelos cursos deste treinador
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => { setOpen(false); resetState() }} />
          <div className="relative w-full max-w-md bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl p-6 space-y-5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Adicionar Membro de Staff</h2>
              <button onClick={() => { setOpen(false); resetState() }} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <form ref={formRef} action={action} className="space-y-4">
              ...
              {/* Treinador → pesquisar utilizador da plataforma */}
              {isTreinador && (
                <div className="space-y-2">
                  <label className="text-xs font-medium text-gray-500 dark:text-gray-400">
                    Treinador registado na plataforma <span className="text-red-400">*</span>
                  </label>
                  <div className="px-3 py-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                    <p className="text-xs text-amber-400">
                      ⚠️ Apenas treinadores registados na plataforma com cursos válidos.
                    </p>
                  </div>
                    ...
                    {showDropdown && userSearch.length > 0 && (
                      <div className="absolute z-20 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                        {filteredUsers.length > 0 ? filteredUsers.map(u => (
                          ...
                          <p className="px-4 py-3 text-sm text-gray-400 text-center">Nenhum utilizador encontrado.</p>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <input type="hidden" name="nome" value={selectedUser?.name ?? ''} />

                  {selectedUser && (
                    <div className="flex items-center gap-2 px-3 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                      <UserCheck size={14} className="text-emerald-400 flex-shrink-0" />
                      <div className="flex-1">
                        <p className="text-sm font-medium text-emerald-400">{selectedUser.name}</p>
                        <p className="text-xs text-gray-400">{selectedUser.email}</p>
                      </div>
                      {loadingEscaloes && <Loader2 size={14} className="text-gray-400 animate-spin" />}
                    </div>
                  )}

                  {/* Escalões cobertos */}
                  {selectedUser && !loadingEscaloes && (
                    <div className="px-3 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg">
                      <p className="text-xs text-gray-500 dark:text-gray-400 mb-1.5">Escalões cobertos pelo curso:</p>
                      {escaloesTreinador.length > 0 ? (
                        <div className="flex flex-wrap gap-1.5">
                          {escaloesTreinador.map(e => (
                            <span key={e} className="px-2 py-0.5 bg-violet-500/10 border border-violet-500/20 text-violet-400 text-xs rounded-full">
                              {e}
                            </span>
                          ))}
                        </div>
                      ) : (
                        <p className="text-xs text-red-400">⚠️ Este utilizador não tem cursos registados na plataforma.</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              {/* Nome livre (funções não-treinador) */}
              {!isTreinador && funcao && (
                <div className="space-y-1">
                  <label className="text-xs font-medium text-gray-500 dark:text-gray-400">
              ...
              {/* Equipa — filtrada pelos escalões do treinador se aplicável */}
              {funcao && (
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {equipasFiltradas.map(e => (
                      <option key={e.id} value={e.id}>
                        {e.nome}{e.escalao ? ` — ${e.escalao}` : ''}
                      </option>
                    ))}
                  </select>
                  {isTreinador && selectedUser && escaloesTreinador.length > 0 && equipasFiltradas.length < equipas.length && (
                    <p className="text-xs text-gray-400">
                      A mostrar apenas equipas dos escalões cobertos pelo curso deste treinador.
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => { setOpen(false); resetState() }}
                  className="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isPending || (isTreinador && (!selectedUser || escaloesTreinador.length === 0)) || !funcao}
