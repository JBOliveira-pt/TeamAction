import { fetchMensalidadesResponsavel } from "@/app/lib/data";
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
                ...
                <div className="bg-white dark:bg-gray-900 rounded-xl p-5 border border-gray-200 dark:border-gray-800 flex flex-col gap-1">
                    ...
                    <span className="text-2xl font-bold text-emerald-500">{pagas}</span>
                    <span className="text-xs text-gray-400">em dia</span>
                </div>
                <div className="bg-white dark:bg-gray-900 rounded-xl p-5 border border-gray-200 dark:border-gray-800 flex flex-col gap-1">
                    <span className="text-xs text-gray-500">Pendentes</span>
                    <span className={`text-2xl font-bold ${pendentes > 0 ? "text-amber-500" : "text-gray-900 dark:text-white"}`}>{pendentes}</span>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                {mensalidades.length === 0 ? (
                    <div className="p-12 flex flex-col items-center gap-3 text-center">
                        ...
                        <p className="text-sm text-gray-500">Nenhuma mensalidade registada.</p>
                    </div>
                ) : (
                    <table className="w-full text-sm">
                        <thead className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
                            <tr>
                                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Mês</th>
                                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Valor</th>
                                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Data pagamento</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                            {mensalidades.map((m) => (
                                <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                    <td className="px-5 py-4 font-medium text-gray-900 dark:text-white">{mesesNomes[m.mes]} {m.ano}</td>
                                    <td className="px-5 py-4 text-gray-600 dark:text-gray-300">{m.valor.toFixed(2).replace(".", ",")} €</td>
                                    <td className="px-5 py-4 text-gray-600 dark:text-gray-300">
                                        {m.data_pagamento ? new Date(m.data_pagamento).toLocaleDateString("pt-PT") : "—"}
                                    </td>
                                    <td className="px-5 py-4">
                                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                                            ...
                                            {m.estado === "pago" ? "Pago" : m.estado === "atrasado" ? "Atrasado" : "Pendente"}
                                        </span>
                                    </td>
                                </tr>
                            ))}
