...
export async function fetchFilteredRecibos(
    ...
    const data = await sql<RecibosTableRow[]>`
        SELECT
            ...
            TO_CHAR(mensalidades.data_pagamento, 'DD/MM/YYYY') AS data_pagamento,
            recibos.status,
            recibos.pdf_url,
...
export async function fetchReciboDetail(reciboId: string) {
    ...
    >`
        SELECT
            ...
            recibos.recibo_number,
            recibos.status,
            TO_CHAR(recibos.received_date, 'YYYY-MM-DD') AS received_date,
            recibos.amount,
            recibos.pdf_url,
            TO_CHAR(recibos.sent_at, 'DD/MM/YYYY HH24:MI') AS sent_at,
            recibos.issuer_iban,
            recibos.created_by AS recibo_created_by,
